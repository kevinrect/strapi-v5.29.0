 ...
 * Update a token
 */
const update = async (id: string | number, attributes: Partial<AppTokenBody>): Promise<AppToken> => {
...
const computeEffectiveAbility = async ({
  ...
  if (token.type === constants.APP_TOKEN_TYPE.CUSTOM) {
    ...
    const allowedActions = tokenPermissions.map((p) => p.action);
    ...
    const effectivePermissions = userPermissions.filter((perm: Permission) =>
      allowedActions.includes(perm.action)
    );
